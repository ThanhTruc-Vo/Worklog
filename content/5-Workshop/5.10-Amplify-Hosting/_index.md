...
1. In the **App settings** step, ensure the configuration matches your React project settings:
...
1. Then click **Save and deploy**. Amplify will automatically pull the source code, execute the build command, and publish the web interface.
...
1. Select the custom domain previously configured in Route 53 (e.g., `cses2.id.vn`) from the dropdown menu.
2. Configure Subdomains to route your `main` branch and set up a redirect from `www` to the root domain.
3. Click **Configure domain** and save. Amplify will automatically verify the DNS with Route 53 and issue an SSL certificate. Your IDP system is now securely accessible via HTTPS!