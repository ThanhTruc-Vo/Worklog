...
1. Tại bước **App settings**, hệ thống sẽ tự nhận diện framework. Hãy đảm bảo cấu hình khớp với dự án React:
...
1. Chọn **Save and deploy**. Amplify sẽ tự động kéo mã nguồn, chạy lệnh build và xuất bản giao diện Web.
...
1. Nhấn nút **Add domain** ở góc phải phía trên.
![Amplify](/images/5-Workshop/5.10-Amplify-Hosting/amplify-5.png)
1. Chọn tên miền đã thiết lập ở Route 53 (ví dụ: `cses2.id.vn`) từ danh sách thả xuống.
2. Cấu hình Subdomains để định tuyến nhánh `main` và thiết lập Redirect từ `www` về tên miền gốc.
3. Nhấn **Configure domain** và lưu lại. Amplify sẽ tự động xác thực DNS với Route 53 và cấp chứng chỉ SSL. Hệ thống IDP của bạn giờ đây đã sẵn sàng hoạt động an toàn qua HTTPS!