...
1. **Associated AWS resources**: Bấm nút **Add AWS resources**.
...
1. Bấm **Next** để chuyển sang bước tạo quy tắc.
...
1. **Rate limit**: Nhập `100` (Hệ thống sẽ tự động chặn IP nếu phát hiện hơn 100 yêu cầu được gửi đến trong vòng 5 phút).
2. **Action**: Chọn **Block**.
![Create WAF](/images/5-Workshop/5.7-API-Gateway-WAF/waf-4.png)
![Create WAF](/images/5-Workshop/5.7-API-Gateway-WAF/waf-5.png)
1. Bấm **Add rule** để lưu quy tắc này.
2. Ở các bước tiếp theo (Bước 3: Set rule priority và Bước 4: Configure metrics), hãy giữ nguyên cấu hình mặc định và bấm **Next**.
3. Tại Bước 5 (Review and create web ACL), kiểm tra lại toàn bộ thông tin và bấm **Create web ACL** để kích hoạt tường lửa.