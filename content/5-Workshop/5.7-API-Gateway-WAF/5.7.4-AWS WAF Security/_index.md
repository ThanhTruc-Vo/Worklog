...
1. **Associated AWS resources**: Click the **Add AWS resources** button.
...
1. Click **Next** to proceed to the rule creation step.
...
1. **Rate limit**: Enter `100` (The system will block the IP if it detects more than 100 requests within a 5-minute window).
...
1. Click **Add rule** to save this configuration.
2. For the subsequent steps (Step 3: Set rule priority, and Step 4: Configure metrics), leave the default settings and click **Next**.
3. In Step 5 (Review and create web ACL), verify your information and click **Create web ACL** to activate the firewall.